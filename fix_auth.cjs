...
authStoreContent = authStoreContent.replace(
    /const response = await authApi\.loginWithGoogle\(\);\s*set\(\{\s*user: response\.user as BackendUser,\s*organization: response\.organization,\s*stations: response\.stations \|\| \[\],\s*currentStation: response\.stations\?\.\[0\] \|\| null,\s*isAuthenticated: true,\s*isLoading: false,\s*error: null,\s*\}\);/g,
...
appTsxContent = appTsxContent.replace(
    /\} else \{\s*\/\/\s*No Firebase user - clear auth state\s*useAuthStore\.setState\(\{\s*isAuthenticated: false,\s*user: null,\s*\}\);\s*\}/g,
    `} else {
